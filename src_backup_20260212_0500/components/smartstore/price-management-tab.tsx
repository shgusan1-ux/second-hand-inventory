        ...
        {paginatedItems.map(p => {
          ...
          return (
            <div key={p.originProductNo} className="bg-white rounded-xl border border-slate-100 p-3">
              <div className="flex items-center gap-2">
                <p className="font-medium text-sm text-slate-800 truncate flex-1">{p.name}</p>
                {p.regDate && (
                  <span className="text-[9px] text-slate-400 shrink-0">{new Date(p.regDate).toLocaleDateString('ko-KR')}</span>
                )}
              </div>
